...
public class MusicNote extends Shape {
     ...
     * Postcondition: Initializes  instance of the Triforce Shape. 
     * @param x
     * @param y
     * @param length
     */
    ...
    public void draw(Graphics g) {
        ...
        shapePrint.fillOval(getX(), getY() + (2 * length / 3), length / 3, length / 3);

        //Draws bottom left circle
        shapePrint.fillOval(getX() + ((2 * length) / 3), getY() + (2 * length / 3), length / 3, length / 3);

        //Draws left verticle bar
        shapePrint.fillRect(getX() + ((4 * length) / 15), getY(), length / 15, (5 * length) / 6);

        //Draws right verticle bar
        shapePrint.fillRect(getX() + ((14 * length) / 15), getY(), length / 15, (5 * length) / 6);

        //Draws top center bar
        shapePrint.fillRect(getX() + ((4 * length) / 15), getY() , length - (length / 3), length / 15);
